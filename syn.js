...
class Keyboard {
    ...
    events() {
        ...
        const isKey = ev => !ev.repeat && (ev.target.matches('#keyboard button') || ev.key && KB.keyMap[ev.key.toUpperCase()]);
        const getHz = ev => ev.type.includes('key') ? KB.keyMap[ev.key.toUpperCase()]?.value : ev.target.value;
        onpointerdown = onkeydown = ev => {
            if (!isKey(ev)) return;
            let hz = getHz(ev);
            console.log(hz);
            hz && Notes.set(hz, new Note(hz));

            onpointerup = onpointercancel = onpointerleave = onpointerout = onkeyup = ev => {
                if (!isKey(ev)) return;
                let hz = getHz(ev);
                Notes.get(hz)?.stop();
                Notes.delete(hz);
                Notes.size || (onpointerup = onpointercancel = onpointerleave = onpointerout = onkeyup = null);
            }
        }
    }
}
